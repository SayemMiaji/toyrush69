...
function OrdersTable({
  ...
  if (!orders?.length) {
    return (
      <div className="border border-black/10 dark:border-white/10 rounded-2xl p-8 text-sm opacity-80">
  ...
  return (
    <main className="container py-8 space-y-8">
      ...
      <div className="mt-6">
        ...
        {tab === "settings" && (
          <section className="rounded-2xl border border-black/10 dark:border-white/10 p-4 text-sm">
            <h2 className="text-lg font-semibold mb-3">Settings</h2>
            {loadingSettings ? (
              <div className="opacity-80">Loading settings…</div>
            ) : settings ? (
              <pre className="whitespace-pre-wrap break-words text-xs bg-white/5 p-3 rounded-lg border border-black/10 dark:border-white/10">
            ...
            {loadingProducts ? (
              ...
              <ul className="space-y-2">
                {products.map((p) => (
                  <li
                    key={p._id || p.id}
                    className="rounded-lg border border-black/10 dark:border-white/10 p-3 flex items-center justify-between"
                  >
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input type="number" min={0} max={100} className="w-20 rounded-md border border-black/10 dark:border-white/10 px-2 py-1 bg-transparent text-xs" value={p.preOrderAdvancePercent ?? 50} onChange={e=>updateProduct(p, { preOrderAdvancePercent: Number(e.target.value) })} />
                      <input className="w-48 rounded-md border border-black/10 dark:border-white/10 px-2 py-1 bg-transparent text-xs" placeholder="Lead time e.g. Ships in 3–4 weeks" value={p.preOrderLeadTimeText || ""} onChange={e=>updateProduct(p, { preOrderLeadTimeText: e.target.value })} />
