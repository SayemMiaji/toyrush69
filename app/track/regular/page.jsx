...
export default function TrackPage(){
  ...
  return (
    <main className="container py-8 space-y-8">
      <div className="rounded-2xl border border-black/10 dark:border-white/10 p-4"><h1 className="text-lg font-semibold mb-1">How to Track Your Orders</h1><ul className="list-disc pl-5 text-sm text-neutral-700 dark:text-neutral-300"><li>Enter the phone number you used when placing the order.</li><li>Complete the quick captcha to confirm you’re human.</li><li>We’ll show all matching orders; click an item to view its current status.</li><li>If you don’t see your order, double‑check your phone digits or contact us on Social media.</li></ul></div>
      ...
      <section className="rounded-2xl border border-black/10 dark:border-white/10 p-4">
        <TrackForm kind="regular" onResult={setOrders} />
      </section>
            ...
            <ul className="grid gap-4">
              {orders.map((o, idx)=>(
                <li key={idx} className="rounded-xl border border-black/10 dark:border-white/10 p-3">
                  ...
                  {o.timeline?.length ? (
                    <div className="mt-4">
                      <StatusPipeline currentStatus={o.currentStatus} history={o.timeline} flow={['Processing','Shipped','Delivered']} />
                    </div>
                  ) : null}
                </li>
              ))}
